...
export function FlashSaleSection({ products = CURATED_DROP_PRODUCTS, onCollect }: {
  ...
  useLayoutEffect(() => {
    ...
    const layout = (animate: boolean) => {
      ...
      const h = mobile ? Math.min(345, Math.round(w * 0.96)) : el.clientHeight;
      const previewHeight = mobile ? 148 : h * .66;
      ...
      cards.forEach((card, index) => {
        ...
        const y = isActive ? 0 : mobile ? h + 14 : h * .21 + (slot - 1) * h * .025;
        ...
        timeline!.to(card.querySelector(".ncd-product"), { scale: isActive ? (mobile ? .92 : 1) : (mobile ? .75 : .85), filter: isActive ? "blur(0px)" : "blur(.35px)" }, 0);
      ...
      <div className="ncd-showcase">
        ...
        <div className="ncd-stage" ref={stage}>
          {products.map((product, index) => <article key={product.id} className="ncd-card" data-active={index === active} aria-label={`${number(index)} / ${products.length}: ${product.name}`}>
            ...
            <div className="ncd-art"><div className="ncd-parallax"><img className="ncd-product" src={product.image} alt={product.name} loading="lazy" decoding="async" draggable={false} /></div><div className="ncd-shadow" /></div>
