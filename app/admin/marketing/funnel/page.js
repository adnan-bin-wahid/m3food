...
export default async function MarketingFunnelPage({ searchParams }) {
  ...
  return <AdminShell admin={admin}>
    <header className="admin-page-header"><div><p className="admin-eyebrow">{result.store.name} · Growth</p><h1>Conversion funnel</h1><p className="admin-muted admin-header-copy">Effy DB funnel from tracked visitor to order, using the same normalized commerce taxonomy wired to Meta and GA4.</p></div><nav className="admin-range-picker">{MARKETING_RANGES.map((option) => <Link key={option} href={`/admin/marketing/funnel?range=${option}`} aria-current={range === option ? 'page' : undefined}>{option === 'all' ? 'All' : option}</Link>)}</nav></header>
    ...
    <section className="admin-panel"><div className="admin-panel-heading"><div><p className="admin-eyebrow">Outcome</p><h2>Visitor → order conversion</h2></div></div><div className="admin-big-conversion"><strong>{result.conversionRate.toFixed(1)}%</strong><p>{result.orders.orders} orders from {result.events.visitors} tracked visitors. Purchase events: {result.events.purchases}.</p></div></section>
  </AdminShell>;
}
