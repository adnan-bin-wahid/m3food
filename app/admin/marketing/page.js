  ...
  return (
    <AdminShell admin={admin}>
      ...
      <section className="admin-metric-grid" aria-label="Marketing summary">
        ...
        <article className="admin-metric-card admin-metric-card-accent"><span>Orders</span><strong>{num(result.orders.orders)}</strong><small>{result.conversionRate.toFixed(1)}% visitor conversion</small></article>
      ...
      <section className="admin-panel admin-marketing-status-panel">
        ...
        {result.statuses.length ? <div className="admin-status-grid">{result.statuses.map((row) => <article key={row.status}><span>{row.status.replaceAll('_', ' ')}</span><strong>{num(row.orders)}</strong><small>{money(row.totalMinor, result.store.currency)}</small></article>)}</div> : <p className="admin-empty">No orders in this period.</p>}
      </section>
      ...
      <section className="admin-panel admin-marketing-source-preview">
        ...
        {result.sources.length ? <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Source</th><th>Visitors</th><th>Views</th><th>ATC</th><th>Checkout</th><th>Orders</th><th>Revenue</th></tr></thead><tbody>{result.sources.slice(0, 8).map((source) => <tr key={source.source}><td><strong>{source.source}</strong><small>{source.medium || '—'}</small></td><td>{num(source.visitors)}</td><td>{num(source.productViews)}</td><td>{num(source.addToCarts)}</td><td>{num(source.checkouts)}</td><td>{num(source.orders)}</td><td>{money(source.revenueMinor, result.store.currency)}</td></tr>)}</tbody></table></div> : <p className="admin-empty">No source activity in this period.</p>}
      </section>
    </AdminShell>
  );
}
