...
export class DrizzleAdminOrderRepository implements AdminOrderRepository {
  ...
  async listOrders(storeId: string, query: AdminOrderListQuery) {
    ...
    if (query.endAt) conditions.push(lt(orders.createdAt, query.endAt));
  ...
  async getOrderDetail(storeId: string, publicId: string) {
    const [base] = await this.database
      .select({
        ...
        phoneVerificationChallengeId: orders.phoneVerificationChallengeId,
        phoneVerifiedAt: orders.phoneVerifiedAt,
        riskLevel: orders.riskLevel,
        riskReasons: orders.riskReasons,
        riskSnapshot: orders.riskSnapshot,
        manualReviewRequired: orders.manualReviewRequired,
    ...
    return {
      ...
      phoneVerificationChallengeId: base.phoneVerificationChallengeId,
      phoneVerifiedAt: base.phoneVerifiedAt,
      riskLevel: base.riskLevel,
      riskReasons: base.riskReasons,
      riskSnapshot: base.riskSnapshot,
      manualReviewRequired: base.manualReviewRequired,
